...
void Computer::execute()
{
     ...
     while(!cpu.halted())
    {
		...
		if((cpu.parsingFunctionInstruction(cpuprog[(cpu.getPC())])==4
                   ||cpu.parsingFunctionInstruction(cpuprog[(cpu.getPC())])==5
                   ||cpu.parsingFunctionInstruction(cpuprog[(cpu.getPC())])==7)
                         &&cpu.getOption()!=0)
		{
                    cout<<cpuprog[(cpu.getPC())]<<"-";
            ...
            cpu.execute(cpuprog[(cpu.getPC())],mem);
